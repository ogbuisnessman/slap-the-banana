...
// bananas = integer
// ownedIndex = highest purchased index (0 = none purchased), we use next to-buy = ownedIndex
// ppc = bananas per click
...
(() => {
  ...
  let ownedIndex = loadInt('ownedIndex', -1); // -1 = nothing bought yet

  function updateUI() {
    bananaCountEl.textContent = bananas;
    bananaCountEl2.textContent = bananas;
    ppcEl.textContent = ppc;
    document.querySelectorAll('.status, #banana-count').forEach(el => {
      // ensure topbar banana count updated (shop uses same key)
    });
  }

  updateUI();

  // We will create several CSS "hand" variants using SVG data-URIs so we can randomize
  const handVariants = [
    // simple open hand (yellow)
  ...
  // animation function: show a random hand that swoops and fades
  function doHandAnimation() {
    ...
    // position random left offset near banana
    const stage = document.getElementById('banana-stage').getB
